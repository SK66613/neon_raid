...
export class GameSimulation {
  ...
  damageBoss(amount = 100, { flashDuration = 0.06 } = {}) {
    if (this.state.stage !== 2 || this.state.won) return;
    const b = this.state.boss; b.hp = Math.max(0, b.hp - Math.max(0, amount)); b.flashTimer = flashDuration;
  ...
  #bossPattern() { const b=this.state.boss;this.#updateBossPhase();if(b.phase===1){this.#bossBurst(5,.72,165);if(this.rng.next()<.65)this.#danger(55,30,35,325,280,490,30,1);b.attackTimer=.82}else if(b.phase===2){this.rng.next()<.5?this.#bossBurst(7,.98,178):this.#bossRadial(11,136);this.#danger(75,46,32,328,275,492,32,.82);b.attackTimer=.68}else{this.rng.next()<.48?this.#bossRadial(15,146):this.#bossBurst(9,1.22,188);this.#danger(90,54,30,330,270,494,34,.65);this.#danger(90,54,30,330,270,494,28,.78);b.attackTimer=.54} }
  ...
  #danger(xRange,yRange,minX,maxX,minY,maxY,radius,delay) { const p=this.state.player;this.state.dangerZones.push({x:clamp(p.x+(this.rng.next()-.5)*xRange,minX,maxX),y:clamp(p.y+(this.rng.next()-.5)*yRange,minY,maxY),radius,delay,life:delay+.22}); }
