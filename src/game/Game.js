...
import { createNetworkGameSession } from './session/NetworkGameSession.js';
import { createNetworkView } from './presentation/networkView.js';
...
export function startGame() {
  ...
  async function run() {
    ...
    const networkMode = params.get('coop') === 'create' || params.get('coop') === 'join' || params.has('room');
    let session;
    try {
      if (networkMode) {
      ...
      } else session = createLocalGameSession();
    } catch (error) { ui.status.textContent = `CO-OP ERROR — ${error.message}`; window.__NEON_READY = true; return; }
    const testAdapter = session.createTestAdapter?.();
    const keys = { up: false, down: false, left: false, right: false };
    const visual = { time: 0, shake: 0, muzzle: 0, muzzles: new Map(), bossFlash: 0, particles: [], casings: [], rain: [], last: 0 };
    ...
    function effects(events) {
      for (const event of events) {
        if (event.type === 'shot-fired') { const angle=-Math.PI/2+DIRS.indexOf(event.direction)*Math.PI/4,raider=event.slot?session.getSnapshot()?.players?.find(player=>player.slot===event.slot):null,x=event.x??raider?.x,y=event.y??raider?.y;if(Number.isFinite(x)&&Number.isFinite(y))visual.casings.push({x,y:y-8,vx:-Math.sin(angle)*(35+Math.random()*28),vy:-30-Math.random()*35,life:.65});visual.muzzle=.06;if(event.slot)visual.muzzles.set(event.slot,.06);tone(90,.035,'sawtooth',.018); }
        if (event.type === 'enemy-shot') { const boss=event.boss||event.bossId==='warden-x';if(boss)visual.bossFlash=.1;tone(boss?50:70,boss ? .07 : .03,boss?'sawtooth':'square',boss ? .025 : .008);  }
        if (event.type === 'player-hit' && (!networkMode || event.slot===session.getConnectionInfo().slot)) { visual.shake=4; tone(45,.08,'sawtooth',.02); }
    ...
    function draw(s) {
      ...
      const drawRaider=(raider,remote=false)=>{if(!raider.alive)return;const movingRaider=Math.hypot(raider.vx,raider.vy)>13,raiderFrame=movingRaider?(Math.floor(visual.time*10)%2?'run1':'run2'):'idle';actor(images.player[raider.firing||(networkMode?visual.muzzles.get(raider.slot)>0:visual.muzzle>0)?raider.aimDirection:raider.lastDirection][raiderFrame],raider,67,remote?'#e035ff':'#25d8e0',raider.hitTimer>0&&Math.floor(raider.hitTimer*22)%2===0?.45:raider.dashTimer>0?.72:remote?.88:1);};
      const drawPlayer=()=>drawRaider(p);
      if(s.stage===1){const actors=[...s.enemies.filter(e=>e.deadTimer<=0).map(enemy=>({y:enemy.y,enemy})),{y:p.y,player:true}].sort((a,b)=>a.y-b.y);for(const entry of actors){if(entry.player){drawPlayer();continue}const e=entry.enemy,enemyMoving=e.burst<=0,enemyFrame=enemyMoving?(Math.floor((visual.time+e.x*.01)*8)%2?'run1':'run2'):'idle';actor(images.enemy[e.burst>0?e.direction:e.moveDirection][enemyFrame],e,62,'#ff334c',e.hitTimer>0?.65:1);ctx.fillStyle='#351016';ctx.fillRect(e.x-17,e.y-35,34,3);ctx.fillStyle='#f03c51';ctx.fillRect(e.x-17,e.y-35,34*Math.max(0,e.hp/e.maxHp),3);if(e.burst>0)drawMuzzle(e.x,e.y-12,e.direction,30)}}else if(networkMode){for(const raider of [p,...s.remotePlayers].sort((a,b)=>a.y-b.y))drawRaider(raider,raider.slot!==p.slot)}else drawPlayer();
      if(networkMode){for(const raider of [p,...s.remotePlayers])if(visual.muzzles.get(raider.slot)>0)drawMuzzle(raider.x,raider.y-12,raider.aimDirection,34)}else if(visual.muzzle>0)drawMuzzle(p.x,p.y-12,p.aimDirection,34);
    ...
    function loop(timestamp) { const dt=Math.min(.033,(timestamp-visual.last)/1000||0);visual.last=timestamp;visual.time+=dt;visual.shake=Math.max(0,visual.shake-dt*17);visual.muzzle=Math.max(0,visual.muzzle-dt);for(const [slot,time] of visual.muzzles)visual.muzzles.set(slot,Math.max(0,time-dt));visual.bossFlash=Math.max(0,visual.bossFlash-dt);for(const casing of visual.casings){casing.vy+=90*dt;casing.x+=casing.vx*dt;casing.y+=casing.vy*dt;casing.life-=dt}visual.casings=visual.casings.filter(casing=>casing.life>0);for(const r of visual.rain){r.y+=r.speed*dt;r.x-=r.speed*.18*dt;if(r.y>550){r.y=-10;r.x=Math.random()*360}if(r.x<-10)r.x=370}for(const particle of visual.particles){particle.life-=dt;if(particle.type!=='smoke')particle.r=(particle.r||4)+((particle.radius||particle.max||28)-(particle.r||4))*Math.min(1,dt*12);else particle.r+=(particle.max-particle.r)*Math.min(1,dt*12)}visual.particles=visual.particles.filter(particle=>particle.life>0);const authoritative=session.getSnapshot();const alive=!networkMode||authoritative?.players.find(player=>player.slot===session.getConnectionInfo().slot)?.alive;if(alive)session.submit(movement());session.update(dt);effects(session.drainEvents());const snapshot=networkMode?createNetworkView(session.getRenderSnapshot(),session.getConnectionInfo().slot):session.getSnapshot();if(snapshot){sync(snapshot);draw(snapshot)}else if(networkMode&&session.getStatus()==='waiting')ui.status.textContent='CO-OP ROOM READY — SHARE THIS URL';requestAnimationFrame(loop); }
    ...
    document.getElementById('dash').addEventListener('click',()=>command({type:CommandType.DASH}));document.getElementById('grenade').addEventListener('click',()=>command({type:CommandType.GRENADE}));document.getElementById('pause').addEventListener('click',()=>networkMode?ui.status.textContent='SERVER-AUTHORITATIVE CO-OP CANNOT BE PAUSED':command({type:CommandType.PAUSE}));document.getElementById('reset').addEventListener('click',()=>networkMode?ui.status.textContent='CO-OP REMATCH IS NOT AVAILABLE':command({type:CommandType.RESTART}));
