...
export function startGame() {
  ...
  function run() {
    ...
    function draw(s) {
      ...
      for(const e of s.enemies.filter(e=>e.deadTimer<=0)){const moving=e.burst<=0,frame=moving?(Math.floor((visual.time+e.x*.01)*8)%2?'run1':'run2'):'idle';actor(images.enemy[e.burst>0?e.direction:e.moveDirection][frame],e,62,e.hitTimer>0?.65:1);ctx.fillStyle='#351016';ctx.fillRect(e.x-17,e.y-35,34,3);ctx.fillStyle='#f03c51';ctx.fillRect(e.x-17,e.y-35,34*Math.max(0,e.hp/e.maxHp),3);if(e.burst>0)drawMuzzle(e.x,e.y-12,e.direction,30);}
      const p=s.player,moving=Math.hypot(p.vx,p.vy)>13,frame=moving?(Math.floor(visual.time*10)%2?'run1':'run2'):'idle';actor(images.player[p.firing||visual.muzzle>0?p.aimDirection:p.lastDirection][frame],p,67,p.hitTimer>0&&Math.floor(p.hitTimer*22)%2===0?.45:p.dashTimer>0?.72:1);
