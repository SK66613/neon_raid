...
export function startGame() {
  ...
  function run() {
    ...
    const visual = { time: 0, shake: 0, muzzle: 0, particles: [], rain: [], last: 0 };
    ...
    function effects(events) {
      for (const event of events) {
        if (event.type === 'shot-fired') { visual.muzzle=.06; tone(90,.035,'sawtooth',.018); }
        if (event.type === 'enemy-shot') tone(70,.03,'square',.008);
        if (event.type === 'player-hit') { visual.shake=4; tone(45,.08,'sawtooth',.02); }
        if (event.type === 'enemy-killed') { visual.particles.push({type:'burst',x:event.x,y:event.y,life:.3}); tone(52,.08,'square',.018); }
        if (event.type === 'explosion') { visual.shake=7; visual.particles.push({...event,type:'explosion',life:.45}); tone(38,.16,'sawtooth',.035); }
        if (event.type === 'dash') { visual.particles.push({...event,type:'dash',life:.22}); tone(190,.06,'triangle',.02); }
        if (event.type === 'boss-smoke') visual.particles.push({...event,type:'smoke',life:.65});
        if (event.type === 'boss-defeated') tone(260,.3,'sawtooth',.035);
        if (event.type === 'wave-started') ui.status.textContent = 'Вторая группа вошла в сектор.';
        ...
        if (event.type === 'reload-started') ui.status.textContent = 'RELOADING…';
    ...
    function floor(stage) { const pattern=ctx.createPattern(images.other.floor,'repeat');ctx.fillStyle=pattern||'#071017';ctx.fillRect(0,0,360,540);ctx.fillStyle='rgba(3,7,11,.45)';ctx.fillRect(0,0,360,540);ctx.fillStyle=stage===1?'rgba(0,196,220,.1)':'rgba(230,35,65,.12)';ctx.fillRect(0,0,360,540);ctx.fillStyle='#11181e';ctx.fillRect(0,0,17,540);ctx.fillRect(343,0,17,540); }
    function actor(image,o,height,alpha=1) { ctx.fillStyle='rgba(0,0,0,.3)';ctx.beginPath();ctx.ellipse(o.x,o.y+7,o.radius*1.35,o.radius*.55,0,0,Math.PI*2);ctx.fill();bottom(image,o.x,o.y+20,height,alpha); }
    function draw(s) {
      ctx.save();ctx.translate((Math.random()-.5)*visual.shake,(Math.random()-.5)*visual.shake);floor(s.stage);
      if(s.stage===1){for(const c of s.crates)bottom(images.other.crate,c.x+c.w/2,c.y+c.h+8,c.h+34);for(const b of s.barrels)if(b.alive)bottom(images.other.barrel,b.x,b.y+27,66);}
      else { const b=s.boss,index=b.attackFrame>=0?Math.min(3,Math.floor(b.attackFrame)):Math.floor(b.frameTimer*5)%4;bottom(b.attackFrame>=0?images.bossAttack[index]:images.bossIdle[index],b.x,b.y+118,250,b.flashTimer>0?.6:1); }
      for(const m of s.medkits)if(m.alive)bottom(images.other.medkit,m.x,m.y+20,48);
      for(const e of s.enemies.filter(e=>e.deadTimer<=0)){const moving=e.burst<=0,frame=moving?(Math.floor((visual.time+e.x*.01)*8)%2?'run1':'run2'):'idle';actor(images.enemy[e.burst>0?e.direction:e.moveDirection][frame],e,62,e.hitTimer>0?.65:1);ctx.fillStyle='#351016';ctx.fillRect(e.x-17,e.y-35,34,3);ctx.fillStyle='#f03c51';ctx.fillRect(e.x-17,e.y-35,34*Math.max(0,e.hp/e.maxHp),3);}
      const p=s.player,moving=Math.hypot(p.vx,p.vy)>13,frame=moving?(Math.floor(visual.time*10)%2?'run1':'run2'):'idle';actor(images.player[p.firing?p.aimDirection:p.lastDirection][frame],p,67,p.hitTimer>0&&Math.floor(p.hitTimer*22)%2===0?.45:p.dashTimer>0?.72:1);
      if(visual.muzzle>0&&images.other.flash?.width){const angle=-Math.PI/2+DIRS.indexOf(p.aimDirection)*Math.PI/4;ctx.save();ctx.translate(p.x,p.y-12);ctx.rotate(angle);ctx.globalCompositeOperation='lighter';ctx.drawImage(images.other.flash,15,-9.5,42.5,19);ctx.restore();}
      ...
      for(const z of s.dangerZones){ctx.globalAlpha=.35+.2*Math.sin(z.life*22);ctx.fillStyle='#f52947';ctx.beginPath();ctx.arc(z.x,z.y,z.radius,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;}
      for(const f of visual.particles){ctx.strokeStyle=f.type==='dash'?'#36dce6':'#ffb038';ctx.lineWidth=4;ctx.beginPath();ctx.arc(f.x,f.y,(1-f.life)*40+5,0,Math.PI*2);ctx.stroke();}
      ...
      if(s.paused){ctx.fillStyle='rgba(0,0,0,.62)';ctx.fillRect(45,226,270,92);ctx.textAlign='center';ctx.fillStyle='#fff';ctx.font='bold 18px monospace';ctx.fillText(s.won?'RAID COMPLETE':s.dead?'RAID FAILED':'PAUSED',180,260);ctx.textAlign='left'}
    }
    function loop(timestamp) { const dt=Math.min(.033,(timestamp-visual.last)/1000||0);visual.last=timestamp;visual.time+=dt;visual.shake=Math.max(0,visual.shake-dt*17);visual.muzzle=Math.max(0,visual.muzzle-dt);for(const r of visual.rain){r.y+=r.speed*dt;r.x-=r.speed*.18*dt;if(r.y>550){r.y=-10;r.x=Math.random()*360}if(r.x<-10)r.x=370}for(const p of visual.particles)p.life-=dt;visual.particles=visual.particles.filter(p=>p.life>0);simulation.step(dt,[movement()]);const snapshot=simulation.getSnapshot();effects(simulation.drainEvents());sync(snapshot);draw(snapshot);requestAnimationFrame(loop); }
