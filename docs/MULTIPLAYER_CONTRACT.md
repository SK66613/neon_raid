...
The reconnect protocol remains valid and implemented, but browser opt-in is temporarily disabled by default after a production startup regression. Ordinary production create and join transports omit `reconnect=1`, receive no resume ticket, and therefore use immediate-disconnect semantics. An internal test-only capability keeps the reconnect-capable path covered; it is not a public page parameter. When opted in, `NetworkGameSession` owns the latest server ticket privately and in memory only. On an eligible active-match interruption it freezes the last authoritative presentation, allows only persistent desired move/fire state to change, and runs at most one resume WebSocket attempt at a time inside the original server-advertised deadline. No gameplay is sent before the authenticated synchronization frame.
