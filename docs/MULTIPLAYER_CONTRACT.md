...
An unexpected active disconnect reserves the same room slot and keeps the authoritative match ticking while neutral move and fire intents are applied. An unauthenticated `resume=1` transport must first send the exact `resume` envelope. Valid authentication may rebind the logical membership from either its reservation or its still-present stale active reconnect-capable transport. Live takeover neutralizes persistent input, retires and strips the old transport before closing it, and makes late old messages, close callbacks, and error callbacks no-ops. Exactly one active transport owns the membership afterward. Successful authentication preserves `roomId`, `matchId`, `connectionId`, slot, host, and simulation state, rotates the secret, and sends an immediate non-advancing synchronization frame. Input `seq` is per WebSocket, so the resumed connection resets to `-1` and may begin at zero; the survivor's sequence is unchanged. Expiry aborts with `player-left`. Runtime loss still fails closed because hosts and reservations remain in memory.
...
Public browser co-op now opts into reconnectable membership: `Game.js` explicitly enables the capability, so fresh production create and join transports include `reconnect=1`. The lower-level `NetworkGameSession` constructor default remains `false`, and the capability is not a public page parameter. `NetworkGameSession` owns the latest server ticket privately and in memory only. On an eligible active-match interruption it freezes the last authoritative presentation, allows only persistent desired move/fire state to change, and runs at most one resume WebSocket attempt at a time inside the original server-advertised deadline. In addition to native WebSocket error and close events, a reconnect-capable `READY` active client refreshes a 1,500 ms liveness watchdog after every accepted authoritative state frame; a stall retires the still-apparently-OPEN socket and enters the same resume machinery. The watchdog is disabled outside an active `READY` match and is generation-bound so an old timer cannot retire a resumed transport. No gameplay is sent before the authenticated synchronization frame. Its lifecycle remains `READY → RECONNECTING → READY` after a successful resume.
...
## Executable cross-layer contract

The deterministic cross-layer test connects the actual `NetworkGameSession` through the actual Worker router to `RaidRoom` and `AuthoritativeMatchHost`, then returns their real protocol-v2 messages to the client. It validates both the lower-level default ordinary startup and the reconnect-capable startup used by public co-op, plus active resume compatibility. Test-only Durable Object, WebSocket, response, clock, timer, and event-queue shims provide infrastructure without changing the runtime contract.
